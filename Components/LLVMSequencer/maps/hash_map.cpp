...
hash_map::hash_map(const bpf_map_def& map_def, I32& res) noexcept :
    mem(pooled_hash_map(map_def.max_entries, map_def.key_size, map_def.value_size, res)) { }

void *hash_map::lookup_elem(const void *key) noexcept {
    return mem.lookup(static_cast<const U8 *>(key));
}
...
long hash_map::delete_elem(const void *key) noexcept {
    return mem.remove(static_cast<const U8 *>(key));
}
